...
import { captureFingerprintUniversal, getBiometricStatus, detectBiometricCapabilities } from '../utils/biometricUtils';

const SessionLocked = ({ open, onResume, onExit }) => {
  ...
  useEffect(() => {
    ...
    const detect = async () => {
      try {
        ...
        setDeviceStatus({ available: false, message: 'No fingerprint sensor detected', deviceType: 'Unknown' });
      }
    };
    detect();
  }, [open]);
  ...
  const handleResume = async () => {
    if (!deviceStatus.available) return;
    setLoading(true);
    try {
      const fingerprintData = await captureFingerprintUniversal();
      const response = await authenticateUser(fingerprintData);
      if (response?.data?.enabled) {
        onResume && onResume(response.data);
      }
    } catch (err) {
      // ignore - keep locked and show no intrusive errors here
      console.error('Resume fingerprint failed:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Box
      sx={{
        position: 'fixed',
        top: 90, // leave app header (approx 90px) undimmed
        left: 0,
        right: 0,
        bottom: 0,
        zIndex: 1400,
      ...
      >
         ...
         <Box sx={{ display: 'flex', alignItems: 'center', width: 718, height: 40, gap: '12px' }}>
           ...
           <Box sx={{ width: 40, height: 40, bgcolor: 'transparent', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
             {/* header icon: circle with provided stroke SVG converted to JSX */}
             <svg width="40" height="40" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="fingerprint circle with strokes">
               ...
               <g transform="translate(8.5,8) scale(1.2)">
           ...
           <Box sx={{ display: 'flex', alignItems: 'center', width: 591, height: 36 }}>
             <Typography sx={{ fontFamily: 'Gallix, sans-serif', fontWeight: 600, fontSize: '24px', lineHeight: '36px', letterSpacing: '-3%', color: '#000048' }}>
               Session Locked - Use Fingerprint to Resume
             </Typography>
           </Box>

           {/* close icon */}
           <Box sx={{ marginLeft: 'auto' }}>
             <IconButton onClick={() => onExit && onExit()} sx={{ width: 18, height: 18, color: '#000048' }} size="small" aria-label="close">
         ...
         <Box sx={{ width: 702, height: 230, mt: '12px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '64px' }}>
           ...
           {/* instruction text */}
           <Box sx={{ width: 702, mt: '12px', display: 'flex', alignItems: 'flex-start', justifyContent: 'flex-start' }}>
             <Typography sx={{ fontFamily: 'Gallix, sans-serif', fontWeight: 400, fontSize: '17px', lineHeight: '26px', letterSpacing: '-3%', color: '#000048', textAlign: 'left' }}>
               {deviceStatus.available ? 'Your session is paused. Place your finger on the scanner to resume.' : deviceStatus.message}
             </Typography>
           </Box>
           ...
           <Box sx={{ width: 64, height: 92, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
             {deviceStatus.available ? (
               ...
               >
                 ...
                 {/* fingerprint SVG centered with gradient (replaced per provided SVG) */}
                 {loading ? (
               ...
               <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
                 <Typography sx={{ fontFamily: 'Poppins, sans-serif', fontWeight: 400, fontSize: '15px', lineHeight: '22px', color: '#000048', textAlign: 'center' }}>{deviceStatus.message}</Typography>
                 <Button variant="contained" onClick={() => onExit && onExit()} sx={{ mt: 1, bgcolor: '#000048', color: '#fff' }}>Exit</Button>
               </Box>
             )}
           </Box>
         </Box>
