...
const WelcomeKiosk = ({ onLogin, onRegister }) => {
  ...
  // 🔹 Controls the background animation visibility
  ...
  const handleFingerprintScan = async () => {
    ...
    try {
      ...
      setShowBgEffort(false); // 🔹 stop background animation after scan finishes
...
<Typography
          ...
          sx={{
            ...
            fontSize: { xs: '1.45rem', sm: '1.65rem', md: '3rem' },
...
<Typography
          ...
          sx={{
            ...
            fontSize: { xs: '0.82rem', sm: '0.9rem', md: '1.6rem' },
...
<Box sx={{ display: 'flex', justifyContent: 'center', mb: { xs: 1.4, sm: 1.8 } }}>
<Box
            ...
            sx={{
              ...
              position: 'relative',   // 🔹 allow background layer
            ...
            onClick={() => {
              ...
              if (!loading) {
                ...
                setShowBgEffort(true); // 🔹 show background animation

                handleFingerprintScan(); // start scan
...
<Box
    ...
    sx={{
      ...
      mixBlendMode: 'screen',        // try 'lighten' if you prefer

      opacity: 0.7,                  // adjust to taste (0.5–0.85 works well)
  ...
  />
            ...
            {/* Foreground content (spinner or static fingerprint) */}
...
>
  ...
  {loading ? (
    ...
    // Show animated fingerprint while scanning
<AnimatedFingerprint size={50} />

  ) : (

    // Static fingerprint when idle (full path, no truncation)
<Box
...
<Box sx={{ textAlign: 'center', mb: { xs: 1.2, sm: 1.6 } }}>
<Box
...
<Link
            ...
            sx={{ color: '#007bff', fontWeight: 600, fontSize: { xs: '0.75rem', sm: '0.8rem' } }}
>
