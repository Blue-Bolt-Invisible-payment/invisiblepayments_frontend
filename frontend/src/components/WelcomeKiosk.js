  ...
  const handleFingerprintScan = async () => {
    ...
    try {
      ...
      if (err.message.includes("cancelled")) {
        ...
        setError(
          "⚠️ Biometric not enrolled. Backend integration required. Please use Test Mode for now."
        );
      } else if (err.message.includes("not recognized")) {
        setError(
          "Fingerprint not recognized. Please try again or contact support."
        );
      } else {
        setError(
          err.message ||
            "Fingerprint authentication failed. Please use Test Mode below."
        );
      }
    }
  };
    ...
    >
      ...
      >
        ...
        >
          <Chip
            icon={
              deviceStatus.available ? <CheckCircleIcon /> : getDeviceIcon()
            }
