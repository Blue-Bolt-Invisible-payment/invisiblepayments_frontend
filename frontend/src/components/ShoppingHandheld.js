...
// Build a fingerprint string from cart items (id + quantity) to detect ANY change
const getCartFingerprint = (items) =>
items.map((ci) => `${ci.id}:${ci.quantity}`).sort().join(",");
...
// Compare fingerprint (covers length change, quantity change, item swap)
const newFingerprint = getCartFingerprint(newItems);
const prevFingerprint = getCartFingerprint(prevCartRef.current);
 
if (newFingerprint !== prevFingerprint) {
 
// RFID AUTO-REMOVE: Detect which item was removed
if (newLength < prevCartRef.current.length && prevCartRef.current.length > 0) {
