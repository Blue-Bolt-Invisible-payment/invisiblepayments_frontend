...
import { getCart, getCartTotal, addItemsToCart, proceedToPay, registerScanner, unregisterScanner } from "../api";
...
/* COMMENTED OUT - RFID indicator hidden from UI, RFID still works in background
const RfidScanningIndicator = ({ active }) => (
<Box
sx={{
display: "flex",
alignItems: "center",
gap: "8px",
bgcolor: active ? "#E8F5E9" : "#FFF3E0",
padding: "8px 16px",
borderRadius: "8px",
border: active ? "1px solid #2DB81F" : "1px solid #FF9800",
}}
>
<Box
sx={{
...
height: "10px",
borderRadius: "50%",
bgcolor: active ? "#2DB81F" : "#FF9800",
animation: active ? "rfidPulse 1.5s ease-in-out infinite" : "none",
"@keyframes rfidPulse": {
...
color: active ? "#2DB81F" : "#FF9800",
}}
>
{active ? "RFID Scanner Active" : "RFID Connecting..."}
</Typography>
</Box>
);
*/
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Right side - Wallet Balance - BESIDE Welcome Message */}
          <Box
            sx={{
              display: "flex",
              alignItems: "center",
        ...
        {/* COMMENTED OUT - RFID indicator and Manual Add button both hidden from UI
<RfidScanningIndicator active={true} />
*/}

        {/* COMMENTED OUT - Add Products Manually button replaced by RFID scanning
<Button
variant="outlined"
...
const ShoppingHandheld = ({
  ...
  /* COMMENTED OUT - Manual Add state replaced by RFID scanning
  ...
  */
  ...
  // RFID scanning state - set to true by default because backend auto-starts scanning via @PostConstruct
  ...
  // MULTI-USER: Register this user as the active scanner when component mounts
  // Unregister when component unmounts (user navigates away or logs out)
  useEffect(() => {
    if (!userId) return;

    const register = async () => {
      try {
        await registerScanner(userId);
        setRfidActive(true);
        console.log("Scanner registered for userId:", userId);
      } catch (err) {
    ...
    // Cleanup: unregister when component unmounts and clear payment timer
    return () => {
      unregisterScanner(userId).catch((err) =>
        console.error("Failed to unregister scanner:", err)
      );
  ...
  // Backend already auto-starts RFID scanning via CaenRfidService @PostConstruct
  // STOP polling when payment is processing or payment success is showing
  useEffect(() => {
    if (!userId) return;

    // Stop polling during payment flow to prevent cart from showing as empty
    ...
    const pollCart = async () => {
      try {
        ...
        console.error("RFID poll - failed to fetch cart:", err);
        setRfidActive(false);
      }
    };
  ...
  /* COMMENTED OUT - Manual Add products array replaced by RFID scanning
  ...
  */
  ...
  /* COMMENTED OUT - Manual Add handler replaced by RFID scanning */
  ...
  const handlePaymentSuccessClose = async () => {
    // Clear auto-redirect timer if user closes manually
    if (paymentSuccessTimerRef.current) {
      clearInterval(paymentSuccessTimerRef.current);
      paymentSuccessTimerRef.current = null;
    }
    // MULTI-USER: Unregister scanner before logout so next user can register
    try {
      await unregisterScanner(userId);
    } catch (err) {
      console.error("Failed to unregister scanner on payment close:", err);
    }
    ...
    <Box sx={{ bgcolor: "#FFFFFF", minHeight: "100vh", width: "100%" }}>
      ...
      >
        ...
        >
          ...
          >
            {/* COMMENTED OUT - RFID indicator and Manual Add button both hidden from UI
<RfidScanningIndicator active={rfidActive} />
*/}

            {/* COMMENTED OUT - Manual Add toggle button replaced by RFID
<Button
variant="outlined"
...
Total Item : {cart.length}
            ...
            <Box
              sx={{
                ...
                pr: { xs: 2, md: 5 }, // Replaces marginRight: 100px with responsive padding
              }}
            >
              <CartIcon sx={{ fontSize: 28, color: "#000048" }} />
              <Typography
                sx={{
                  fontFamily: "Gallix, sans-serif",
          ...
          {/* COMMENTED OUT - Manual Add product panel replaced by RFID scanning
{showManualAdd && (
...
*/}
        ...
        <Box
          sx={{
            ...
            bgcolor: "#F2F2F2",
            display: "flex",
            flexDirection: "column", // Standard flex column layout
            p: 0, // Set padding to 0 here so the footer can span the full width
          ...
          <Box sx={{ mt: "auto" }}>
            ...
            <Box
              sx={{
                ...
                alignItems: "center",
                p: "16px 24px",
                bgcolor: "#D9D9D9", // Light grey background like Figma
              }}
            >
              <Box>
            ...
            <Button
              ...
              sx={{
                ...
                bgcolor: "#26EFE9",
                color: "#000048",
                borderRadius: 0, // Square edges to match Figma footer
                fontWeight: 700,
                fontSize: "20px",
                textTransform: "none",
...
Your payment has been completed.
            ...
            {/* HIDDEN - 30s auto-redirect timer runs silently in background
<Typography
sx={{
fontFamily: "Gallix, sans-serif",
fontSize: { xs: "12px", md: "14px" },
...
>
Redirecting to home in {successCountdown}s...
</Typography>
*/}
          </Box>
        </DialogContent>
